<template>
  <el-form ref="form"  label-width="80px">
    ...
    <el-form-item label="销售属性">
      <el-form label-width="80px" :inline="true" ref="form" class="demo-form-inline">
        <el-form-item :label="item1.saleAttrName" 
        v-for="item1 in spuSaleAttrList" 
        :key="item1.id">
          <el-select placeholder="请选择" v-model="item1.attrIdAndValueId">
            <el-option :label="item2.saleAttrValueName" 
            v-for="(item2,index) in item1.spuSaleAttrValueList" :key="item2.id" value="value" 
            :value="`${item1.id}:${item2.id}`"></el-option>
          </el-select>
        </el-form-item>
      </el-form>
    </el-form-item>
    <el-form-item label="图片列表" prop="name">
      <el-table border
        ref="multipleTable"
        :data="spuImageList"
        tooltip-effect="dark"
